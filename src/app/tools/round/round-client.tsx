...
export default function RoundClient() {
  ...
  const applyRadius = useCallback(async () => {
    ...
    try {
      ...
      const r = radius // max radius should be half the smaller dimension; we'll clamp
      const clampR = Math.min(radius, w / 2, h / 2)
      ...
      ctx.beginPath()
      ctx.moveTo(clampR, 0)
      ctx.lineTo(w - clampR, 0)
      ctx.quadraticCurveTo(w, 0, w, clampR)
      ctx.lineTo(w, h - clampR)
      ctx.quadraticCurveTo(w, h, w - clampR, h)
      ctx.lineTo(clampR, h)
      ctx.quadraticCurveTo(0, h, 0, h - clampR)
      ctx.lineTo(0, clampR)
      ctx.quadraticCurveTo(0, 0, clampR, 0)
      ctx.closePath()
      ctx.clip()
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          setError('Failed to apply rounded corners.')
        }
        setProcessing(false)
      }, 'image/png')
    } catch {
  ...
  const maxRadius = Math.min(dimensions.w, dimensions.h) / 2
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-4">
          ...
          {preview && (
            <div className="mt-3 flex gap-3 overflow-hidden">
              ...
              {result && (
                <img src={result.url} alt="Rounded" className="h-20 w-20 rounded-lg border border-zinc-200 object-cover" />
              )}
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-4">
          <div className="mb-4">
            <label className="mb-1.5 flex items-center justify-between text-sm font-medium text-zinc-700">
              <span>Corner Radius: {displayRadius.toFixed(0)}px</span>
              <span className="text-xs text-zinc-400">
                {displayRadius >= maxRadius ? 'Circular' : displayRadius > maxRadius * 0.5 ? 'Rounded' : 'Slight'}
              </span>
            </label>
            <input
              type="range"
              min={0}
              max={Math.max(dimensions.w, dimensions.h, 200)}
              step={1}
              value={radius}

...
export default function WatermarkClient() {
  ...
  const applyWatermark = useCallback(async () => {
    ...
    try {
      ...
      const pad = 24
      const fs = Math.min(fontSize, img.naturalWidth * 0.15)
      ...
      switch (position) {
        ...
        case 'top-right': x = img.naturalWidth - tw - pad; y = pad; break
        case 'center': x = (img.naturalWidth - tw) / 2; y = (img.naturalHeight - th) / 2; break
        case 'bottom-left': x = pad; y = img.naturalHeight - th - pad; break
        case 'bottom-right': x = img.naturalWidth - tw - pad; y = img.naturalHeight - th - pad; break
      }
  ...
  }, [preview, text, position, opacity, fontSize])
    ...
    <div className="space-y-6">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-4">
          ...
          {preview && (
            <div className="mt-3 flex gap-3 overflow-hidden">
              ...
              {result && (
                <img src={result.url} alt="Watermarked" className="h-20 w-20 rounded-lg border border-zinc-200 object-cover" />
              )}
            </div>
          )}
        </div>
      ...
      {file && !result && (
        <div className="rounded-xl border border-zinc-200 bg-white p-4">
          {/* Text */}
          <div className="mb-4">
            <label className="mb-1 block text-sm font-medium text-zinc-700">Watermark Text</label>
            <input
              type="text"
              value={text}
          ...
          {/* Position */}
          <div className="mb-4">
            <label className="mb-1.5 block text-sm font-medium text-zinc-700">Position</label>
